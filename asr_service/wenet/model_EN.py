...
class WeNetASREN:
    ...
    def post_process(self, output):
        ...
        tokens = self.vocabulary[token_idx_list]
        text = self.sentencepiece_decode(tokens)
        return text

    def sentencepiece_decode(self, tokens):
        """将分词token列表还原为标准文本（去掉▁，加空格）"""
        sentence = ''
        for token in tokens:
            if token.startswith('▁'):
                if sentence:  # 第一个词不加空格
                    sentence += ' '
                sentence += token[1:]
            else:
                sentence += token
