...
class CausalLMATB(PreTrainedModel):
    ...
    def __init__(self, config: PretrainedConfig, weights: Weights, **kwargs):
        ...
        if hasattr(config, 'rope_scaling') and self.config.rope_scaling is not None:
            self.scaling_factor = self.config.rope_scaling.get("factor", 1.0)
        else:
    ...
    def prepare_inputs_for_generation(
            self, input_ids: torch.Tensor, past_key_values: Optional[torch.Tensor] = None,
            attention_mask: Optional[torch.Tensor] = None, inputs_embeds: Optional[torch.Tensor] = None, **kwargs
    ) -> dict:
        """
        ...
        Args:
            ...
            past_key_values (torch.Tensor, optional): Input past key value tensor, default to None.
            attention_mask (torch.Tensor, optional): Input attention mask tensor, default to None.
            input_embeds (torch.Tensor, optional): Input embedding tensr, default to None.
        """
        if past_key_values:
