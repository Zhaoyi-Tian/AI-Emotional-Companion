...
class FlashForCausalLM(BaseModel):
    ...
    def __init__(self, config: PretrainedConfig, weights: Weights, **kwargs):
        ...
        self.layerskipped_enable = self.inference_mode == InferenceMode.LAYERSKIPPED
        ...
        if self.layerskipped_enable:
            self.layer_skipped_decoder_param = None
            self.part_decoder = False
            self.speculate_enable = True
            self.acl_decoder_operation = None
            self.acl_part_decoder_with_speculate_operation = None
