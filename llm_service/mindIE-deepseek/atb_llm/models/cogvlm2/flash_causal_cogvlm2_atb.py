...
class FlashCogvlm2ForCausalLMATB(FlashForCausalLMATB):
    def __init__(
        ...
        self.init_model(config, weights, model_prefix, lm_head_prefix, backend)
        self.init_vit()
    ...
    def init_model(self, config, weights, model_prefix, lm_head_prefix, backend):
        self.model = CogvlmModel(config, weights, model_prefix, lm_head_prefix, backend)
    ...
    def init_graph(self):
        ...
        self.build_graph(self.prefill_graph, is_prefill=True, with_vision_expert=False)
    ...
    def prepare_prefill_token_service(self, input_ids):
        ...
        for _, bos in enumerate(bos_idxs):
            ...
            if torch.any(torch.eq(input_ids[bos: next_bos], self.image_token_id)):
                ...
                hidden_states = self._merge_input_ids_with_image_features_service(images_features,
                                                                        hidden_states,
                                                                        bos)
    ...
    def forward(
        ...
        **kwargs,
        ...
        with_vision_expert = is_prefill and torch.any(
            torch.eq(input_ids, self.image_token_id)
        )
    ...
    def _merge_input_ids_with_image_features_service(self, images_features, hidden_states, bos):
        hidden_states[bos + 1: bos + _IMG_TOKEN_LEN + 1] = images_features
        return hidden_states
