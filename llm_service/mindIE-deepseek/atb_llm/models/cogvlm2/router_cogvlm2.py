...
from .input_builder_cogvlm2 import Cogvlm2InputBuilder
from .data_preprocess_cogvlm2 import cogvlm2_image_preprocess

_PAD_TOKEN_ID = Cogvlm2Config.pad_token_id
_BOS_TOKEN_ID = Cogvlm2Config.bos_token_id
_EOS_TOKEN_ID = Cogvlm2Config.eos_token_id
_IMG_TOKEN_LEN = Cogvlm2Config.img_token_len


@dataclass
class Cogvlm2Router(BaseRouter):
    def __post_init__(self):
        super().__post_init__()
        self.enable_atb_torch = True
    ...
    def tokenize(self, inputs: List[Dict], **kwargs):
        ...
        if image_path == "":
            ...
            shm_name_list = []
            shape_value_list = []
            image_pixel = cogvlm2_image_preprocess(image_path=image_path)
            ...
            image_token = torch.full((_IMG_TOKEN_LEN, ), pad_token_id, dtype=input_ids.dtype)
            new_input_ids = [torch.tensor([bos_token_id], dtype=input_ids.dtype), image_token, input_ids]
            new_input_ids = torch.cat(new_input_ids)
    ...
    def get_config(self):
        config = Cogvlm2Config.from_dict(self.config_dict)
    ...
    def get_input_builder(self):
        pad_token_id = self.config.pad_token_id if self.config.pad_token_id is not None else _PAD_TOKEN_ID
        return Cogvlm2InputBuilder(self.tokenizer, self.generation_config, pad_token_id)