...
class BaseModel(BasePreTrainedModel):
    ...
    def prepare_inputs_for_ascend(
        ...
        block_tables = torch.zeros((attention_mask.shape[0], position_ids.shape[1]), dtype=torch.int32, device="npu")
    ...
    def _forward(
        ...
        if nonzero_seq_len is None:
            attention_mask = self.kv_attention_manager.get_attention_mask(attention_mask)
