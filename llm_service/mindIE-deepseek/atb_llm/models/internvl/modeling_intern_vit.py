...
class InternVisionEmbeddings(nn.Module):
    ...
    def _get_pos_embed(self, pos_embed, height, width):
        ...
        pos_embed = F.interpolate(pos_embed, size=(height, width), mode='bicubic', align_corners=False). \
                reshape(1, -1, height * width).permute(0, 2, 1).to(dtype=target_dtype, device=target_device)
        return pos_embed
...
class InternAttention(nn.Module):
    ...
    def __init__(self, config: InternVisionConfig, process_group):
        ...
        if self.head_dim * self.num_heads != self.embed_dim:
            logger.error(f'embed_dim must be divisible by num_heads (got `embed_dim` is {self.embed_dim} '
        ...
        self.qkv = ColumnLinear(self.embed_dim, 3 * self.embed_dim, bias=config.qkv_bias, \
    ...
    def _npu_flash_attn(self, x):
        ...
        qkv = self.qkv(x).reshape(batch_size, sequence_length, 3, 
                                  self.num_heads, embedding_size // self.num_heads).permute(2, 0, 3, 1, 4)
        ...
        x = torch_npu.npu_prompt_flash_attention(q, k, v, num_heads=self.num_heads, input_layout="BNSD",
                                                scale_value=head_dim ** -0.5, pre_tokens=65535, next_tokens=65535)
...
class InternMLP(nn.Module):
    def __init__(self, config: InternVisionConfig, process_group):
        ...
        self.fc1 = ColumnLinear(config.hidden_size, config.intermediate_size, \
            gather_output=False, process_group=process_group)
...
class InternVisionModel(PreTrainedModel):
    ...
    def forward(
        ...
        if pixel_values is None and pixel_embeds is None:
            logger.error('You have to specify pixel_values or pixel_embeds, currently it is None.',
                         ErrorCode.ATB_MODELS_PARAM_OUT_OF_RANGE)
        ...
        if pixel_embeds is not None:
            ...
            if len(pixel_values.shape) == 4:
                ...
                logger.error(f'Wrong pixel_values size {pixel_values.shape}, its dimension should be 4.',
                             ErrorCode.ATB_MODELS_PARAM_OUT_OF_RANGE)
