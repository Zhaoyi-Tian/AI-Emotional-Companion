...
from .modeling_mixtral_atb import MixtralModelATB
...
from ...common_op_builders.linear_parallel.base_linear_parallel_common_op_builder \
    import ParallelType, TensorParallelInfo, CommunicationBackend
...
class FlashMixtralForCausalLMATB(FlashForCausalLMATB):
    def __init__(self, config, weights, lm_head_prefix="lm_head", model_prefix="model", **kwargs):
        ...
        self.model = MixtralModelATB(config, weights, model_prefix, lm_head_prefix, 
    ...
    def build_graph(self, graph, is_prefill):
        ...
        for i in range(self.config.num_hidden_layers):
            kv_cache_names.extend([f"layer_{i}_{_K_CACHE}", f"layer_{i}_{_V_CACHE}"])
    ...
    def prepare_inputs(
        ...
        if self.position_embedding_type == "ROPE":
            ...
            logger.error(f"Error: position_embedding_type is inllegal {self.position_embedding_type}", 
        ...
        self.graph_inputs[target_key].update({
            ...
            "expert_array": self.expert_array,
            "expert_group": self.expert_group,
            "one_hot": self.one_hot,
            "zero_hot": self.zero_hot,
        })
