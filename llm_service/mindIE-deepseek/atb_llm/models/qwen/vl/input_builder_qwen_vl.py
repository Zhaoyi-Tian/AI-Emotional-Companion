        ...
        **kwargs):
        ...
        for message in conversation:
            for single_input in message[_CONTENT]:
                if _IMAGE not in single_input.keys():
                    continue
                image_pixel = qwen_vl_image_preprocess(single_input[_IMAGE])
        ...
        query = self.tokenizer.from_list_format(conversation.pop()[_CONTENT])

        for message in conversation[::-1]:
            turn_query = self.tokenizer.from_list_format(message[_CONTENT])
            if message["role"] == self.user_role_name:
                ...
                msg = f"Conversation 'role': '{message['role']}' not supported yet. " + \
                      f"Only '{self.user_role_name}' and '{self.system_role_name}' are supported."
