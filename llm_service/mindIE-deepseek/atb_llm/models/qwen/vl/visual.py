...
from .data_preprocess_qwen_vl import qwen_vl_image_preprocess

_INTERPOLATE_FALL_BACK = torch_npu.npu.get_device_name().startswith("Ascend310P")
_IMAGE_PROCESS_POOL_SIZE = 24
...
def _get_2d_sincos_pos_embed_from_grid(embed_dim, grid):
    if embed_dim % 2 != 0:
        msg = "The output dimension 'embed_dim' must be divided by 2."
...
class Resampler(nn.Module):
    ...
    def __init__(
        ...
        self.pos_embed = nn.Parameter(
            torch.from_numpy(_get_2d_sincos_pos_embed(embed_dim, grid_size)).float()
    ...
    def forward(self, x, attn_mask=None):
        pos_embed = _get_abs_pos(self.pos_embed, x.size(1))
...
class VisualAttention(nn.Module):
    ...
    def __init__(self, embed_dim, num_heads, kdim=None, vdim=None):
        ...
        if embed_dim % num_heads != 0:
            msg = "The output dimension 'embed_dim' must be divided by 'num_heads'."
            logger.error(msg, ErrorCode.ATB_MODELS_PARAM_INVALID)
            raise ValueError(msg)
        ...
        if not self._qkv_same_embed_dim:
            msg = "Only support Self-Attention currently."
            logger.error(msg, ErrorCode.ATB_MODELS_PARAM_INVALID)
            raise ValueError(msg)
...
class VisualAttentionBlock(nn.Module):
    ...
    def attention(
        ...
        return self.attn(q_x, k_x, attn_mask=attn_mask)
...
class VisionTransformer(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        super().__init__()
        self.pool = Pool(_IMAGE_PROCESS_POOL_SIZE)
    ...
    def forward(self, x: torch.Tensor):
        ...
        x = x + _get_abs_pos(self.positional_embedding, x.size(1))
    ...
    def encode(self, image_paths: List[str]):
        images_npy = self.pool.map(qwen_vl_image_preprocess, image_paths)
        images = torch.stack([torch.from_numpy(x) for x in images_npy], dim=0)
        return self(images)
