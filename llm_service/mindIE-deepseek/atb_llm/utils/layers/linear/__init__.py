...
def get_linear(weight, bias, quantize, is_norm=False, **kwargs):
    if quantize is None:
        linear = FastLinear(weight, bias, is_norm, nd_weight=kwargs.get('nd_weight', False))
...
class TensorParallelColumnLinear(SuperLayer):
    ...
    @classmethod
    def load_moe(cls, config, prefixes: List[str], weight, bias: bool, dim: int = 0):
        linear = get_linear(
            weight, bias, config.quantize, prefixes=prefixes, num_linear_before_pack=len(prefixes),
            tensor_parallel_dim=dim, align_size=1
        )
...
class TensorParallelRowLinear(SuperLayer):
    ...
    def load(cls, config, prefix: str, weights, bias: bool, bias_pre_add=False, gqa_size=1,
        dim=1, transpose_b=TransposeType.INVALID, nd_weight=False):
    ...
    @classmethod
    def load_moe(cls, config, prefix: str, process_group, weight, bias: bool):
        linear = get_linear(
            weight, bias, config.quantize, prefixes=[prefix], tensor_parallel_dim=1 # 在RowLinear下，dim为1
        )
