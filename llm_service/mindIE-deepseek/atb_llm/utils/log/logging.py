...
def init_logger(logger_ins: CustomLogger, file_name: str, stream=None):
    ...
    if len(log_file_path) == 0:
        ...
        log_file_path = os.path.join(home_dir, "mindie", "log", log_file_path, "debug")
    llm_log_file_path = os.path.join(log_file_path, f"mindie-llm_atbmodels_{pid}_{process_datetime}.log")
