...
性能测试时需要在 `${image_path}` 下仅存放一张图片，使用以下命令运行 `run_pa.sh`，会自动输出batchsize为`batch_size`时，输出token长度为 256的吞吐。
...
```shell
bash examples/models/glm4v/run_pa.sh --performance --trust_remote_code ${weight_path} ${image_path} ${batch_size} ${max_input_length} ${max_output_length}
```
