...
show_help() {
    ...
    echo "-type TYPE  Specify model tpye and quant type. Support w8a8_dynamic"
...
get_disable_name() {
    ...
    for ((i=0; i<$num_layer; i++)); do
        disable_names="$disable_names model.layers.$i.mlp.gate.wg"
    done
...
case "$type" in
    w8a8_dynamic)
        disable_names=$(get_disable_name 64)
        param="--disable_names $disable_names --part_file_size 5 --device_type cpu --act_method 1 --w_bit 8 --a_bit 8 --is_dynamic True"
        ;;
    *)
...
python -m examples.convert.model_slim.quantifier --model_path $src --save_directory $dst --calib_file "" $param $TRUST_REMOTE_CODE