...
if [[ "$1" == "--trust_remote_code" ]]; then
    ...
    TRUST_REMOTE_CODE=""
    echo "[TRUST_REMOTE_CODE]: set to false defaultly. Please check local files's safety and then set it"
fi
...
if [ "$TP_WORLD_SIZE" == "1" ]; then
    ...
    torchrun --nproc_per_node $world_size --master_port $MASTER_PORT -m examples.run_pa --block_size 128 $extra_param
fi
