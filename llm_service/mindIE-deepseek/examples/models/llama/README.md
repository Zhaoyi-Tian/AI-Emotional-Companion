...
- 环境变量说明
  ...
  - `export ATB_LLM_HCCL_ENABLE=1`
      - 进行流水线并行时，使能HCCL通信后端。LCCL暂不支持
- 运行流水线并行(pipeline parallel) / 混合并行(TP+PP)
  - 运行前需要export的环境变量
    - export MASTER_IP="ip地址"
    - export NETWORK_ADAPTER="网卡"
      - 通过ifconfig去查询ip地址对应的网卡
  - 并行参数说明
    - `TP_SIZE`为张量并行数，`PP_SIZE`为流水线并行数，`MICROBATCH_SIZE`为微批次中的request数量
    - 需满足`TP_SIZE` * `PP_SIZE` = `world_size` (总卡数)
