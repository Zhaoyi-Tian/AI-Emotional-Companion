...
if [ "$TP_WORLD_SIZE" == "1" ]; then
    ...
    torchrun --nproc_per_node $world_size --master_port $MASTER_PORT -m examples.run_pa --model_path "$model_path" \
    --tp $TP_SIZE --pp $PP_SIZE --microbatch_size $MICROBATCH_SIZE --max_batch_size 1 $extra_param
fi