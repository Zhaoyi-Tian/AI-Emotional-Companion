...
##多机场景
### PA场景
bash run.sh pa_[data_type] performance [case_pair] [batch_size] ([prefill_batch_size]) [model_name] ([is_chat_model]) [weight_dir] ([trust_remote_code]) [rank_table_file] [world_size] [node_num] [rank_id_start] [master_address]([max_position_embedding/max_sequence_length])
...
7. parallel_params接受一组输入，格式为[dp,tp,moe_tp,moe_ep,pp,microbatch_size]，如[8,1,1,8,1,1]
    说明：
...
### FA场景
bash run.sh fa full_CEval ([shots]) [batch_size] [model_name] ([is_chat_model]) (lora [lora_data_path]) [weight_dir] ([trust_remote_code]) [chip_num] ([max_position_embedding/max_sequence_length])
...
1. dataset接受一个输入，根据需要从以下列表中选择：
    [full_CEval|full_BoolQ|full_HumanEval|full_HumanEval_X|full_MMLU|full_CMMLU|full_TruthfulQA|full_LongBench|full_GSM8K|full_NeedleBench|edge_BoolQ|edge_GSM8K]
...
7. multi-lora特性仅支持BoolQ，CEval和GSM8K数据集
...
11. parallel_params接受一组输入，格式为[dp,tp,moe_tp,moe_ep,pp,microbatch_size]，如[8,1,8,-1,-1,-1]
    说明：
    1. 使用pp前需要使能HCCL后端，export ATB_LLM_HCCL_ENABLE=1
    2. 使用pp前需要设置ip_address和网卡，export MASTER_IP="ip地址"，export NETWORK_ADAPTER="网卡"，其中网卡可通过ifconfig查询到ip对应的网卡
    3. pp和dp暂不支持混合
